...
model.load_state_dict(torch.load('/data/iccvw/baseline+preprocess+u_deconv/results/09100943/model_checkpoint_epoch_490.pt'))


graph_path = '/data/iccvw/CHALLENGE/dataset_struct_in_aug/test/graph_in/'
filenames = [os.path.splitext(f)[0] for f in os.listdir(graph_path)]

dest_dir = '/data/iccvw/baseline+preprocess+u_deconv/submission_unet_2/'

print(len(filenames))

# Load sample
for filename in filenames:
    graph_path = '/data/iccvw/CHALLENGE/dataset_struct_in_aug/test/graph_in/'+filename+'.pickle'
    struct_path = '/data/iccvw/CHALLENGE/dataset_struct_in_aug/test/struct_in/'+filename+'.npy'
